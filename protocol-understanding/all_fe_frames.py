#!/usr/bin/env python3
"""Show frames just before and during data transfer to see the transition."""
import struct
...
print()
print("ALL FE FRAMES (complete capture):")
print()

t0 = fe_frames[0]['ts']
for i, f in enumerate(fe_frames):
    rel = (f['ts'] - t0) / 1000
    body_hex = f['body'][:20].hex() if f['body'] else ''
    print(f'  #{i:2d}  {rel:8.1f}ms  {f["dir"]} flag=0x{f["flag"]:02x} cmd=0x{f["cmd"]:02x} bodylen={len(f["body"]):3d}  {body_hex}')
